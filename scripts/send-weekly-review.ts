...
async function main() {
  const recipient = process.argv[2] || process.env.GOOGLE_ADMIN_EMAIL || "zach@photonbrothers.com";

  // Force review-only delivery to a single inbox.
  process.env.SCHEDULING_NOTIFICATION_BCC = "";

  console.log(`Sending review batch to: ${recipient}`);

  const weekly = await sendWeeklyChangelogSimpleEmail({
    to: recipient,
  ...
  const opsOnly = await sendOperationsOnlyUpdateEmail({
    to: recipient,
  ...
  const backlog = await sendBacklogForecastingUpdateEmail({
    to: recipient,
  ...
  if (!weekly.success || !opsOnly.success || !backlog.success) {
    process.exitCode = 1;
  }
}
