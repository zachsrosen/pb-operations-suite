/**
 * Google Doc: all owed-but-not-uploaded PE docs (PTO-stage deals owe the 12
 * M1 docs, Close Out deals owe all 15), grouped by owning team → doc type →
 * checkbox per deal with HubSpot / PE / Drive links.
 *
 * Run: npx tsx --env-file=.env scripts/_pe-missing-docs-by-team-gdoc.ts [existingDocId]
 */
...
async function main() {
  ...
  const docs = await prisma.peDocumentReview.findMany({ select: { dealId: true, docName: true, status: true } });
  const m1Set = new Set<string>(PE_M1_DOC_NAMES);

  // docName -> deals missing it
  interface MissingDeal { name: string; hs: string; pe: string; drive: string; m: string }
  ...
  console.log(`missing docs: ${totalDocs} across ${totalDeals} deals`);

  // ---- Build doc content ----
  const title = `PE — Missing Documents by Team (${totalDocs} docs · ${totalDeals} deals) — 2026-06-12\n`;
  const intro = `Owed docs not yet uploaded to the PE portal: PTO-stage deals owe the 12 M1 docs, Close Out deals owe all 15. Check off each as it's uploaded. Team mapping is folder-source based — flag Zach if a doc belongs to a different team.\n`;
  ...
  if (documentId) {
    ...
    const createRes = await fetch("https://docs.googleapis.com/v1/documents", {
      method: "POST", headers, body: JSON.stringify({ title: "PE — Missing Documents by Team (2026-06-12)" }),
    });
