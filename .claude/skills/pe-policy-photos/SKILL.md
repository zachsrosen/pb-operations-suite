...
4. **Orders + embeds** — orders kept photos by the canonical shot sequence and embeds the project's **Zoho Sales Order PDF** at its #6 ("Invoice & BOM") rank. The SO is **located** in the Drive `Participate Energy` folder (then the `0. Sales` folder) by filename — never regenerated; if absent it's flagged and the package is assembled without it.
