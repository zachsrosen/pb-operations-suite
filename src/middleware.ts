...
const PUBLIC_API_ROUTES = [
  ...
  "/api/cron/zuper-property-sync", // Zuper property sync — CRON_SECRET validated in route
  "/api/cron/pe-avg-timing", // PE avg submission→payment writer — CRON_SECRET validated in route
  ...
  "/api/cron/pe-rejection-advance", // PE rejection → Ready to Resubmit poller — CRON_SECRET validated in route
  "/api/cron/pe-doc-digest",       // PE doc status daily digest — CRON_SECRET validated in route
  "/api/cron/pe-clawback-alert",   // PE approved-doc re-open (ANCHOR clawback) alert — CRON_SECRET validated in route
  ...
  "/api/webhooks/hubspot/admin-workflow-property", // HubSpot workflow → admin workflow fan-out — HubSpot signature validated in route
  ...
  "/api/cron/solaredge-sync",       // SolarEdge asset sync — CRON_SECRET validated in route
...
function addSecurityHeaders(requestId: string, response: NextResponse): NextResponse {
  ...
  response.headers.set(
    ...
    "default-src 'self'; script-src 'self' 'unsafe-inline' https://va.vercel-scripts.com https://maps.googleapis.com https://www.google.com https://www.gstatic.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https:; worker-src 'self' blob:; frame-src 'self' https://www.google.com https://atlas.photonbrothers.com; frame-ancestors 'self'; object-src 'none'; base-uri 'self'; form-action 'self';"
  );
