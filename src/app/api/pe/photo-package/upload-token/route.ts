/**
 * PE Photo Package Upload Token API
 *
 * POST /api/pe/photo-package/upload-token
 *   Returns a Vercel Blob client upload token for a PE photo package upload.
 ...
 *
 * Auth required: accounting/ownership/PM roles
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const logUploadToken = async (
    ...
    await logActivity({
      ...
      description:
        outcome === "issued"
          ? "Issued PE photo package upload token"
          : "PE photo package upload token request failed",
      userEmail: authResult.email,
      userName: authResult.name,
      entityType: "pe_photo_package",
      entityName: "upload_token",
      metadata: {
        event: "pe_photo_package_upload_token",
        outcome,
        ...details,
      },
      ipAddress: authResult.ip,
      userAgent: authResult.userAgent,
      requestPath: "/api/pe/photo-package/upload-token",
      requestMethod: "POST",
      responseStatus,
  ...
  try {
    ...
    pathname = body.payload?.pathname ?? body.pathname ?? "pe-photo-package/photos.zip";
  } catch {
    await logUploadToken("failed", { reason: "invalid_json_body" }, 400);
    return NextResponse.json({ error: "Invalid request body" }, { status: 400 });
  }

  // Validate pe-photo-package prefix
  if (!pathname.startsWith("pe-photo-package/")) {
    await logUploadToken("failed", { reason: "invalid_pathname_prefix", pathname }, 400);
    return NextResponse.json({ error: "Pathname must start with pe-photo-package/" }, { status: 400 });
  }

  try {
    // Token valid for 30 minutes — enough for a large photo upload on a slow connection
    const validUntil = Date.now() + 30 * 60 * 1000;

    const clientToken = await generateClientTokenFromReadWriteToken({
      token: process.env.BLOB_READ_WRITE_TOKEN,
      pathname,
      maximumSizeInBytes: 50 * 1024 * 1024, // 50 MB — covers large photo packages
      allowedContentTypes: ["application/pdf", "application/octet-stream", "image/jpeg", "image/png", "application/zip"],
    ...
    console.error("[pe/photo-package/upload-token] Error:", msg);
