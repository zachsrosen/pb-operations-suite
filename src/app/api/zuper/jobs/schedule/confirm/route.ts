          ...
          if (isInstallationConfirm) {
            try {
              ...
              try {
                ...
                console.warn("[Zuper Confirm] Sibling cache lookup failed:", cacheErr);
              }

              // Source 2: Zuper API search (fallback when cache is empty)
              if (siblingMap.size === 0) {
                try {
                  ...
                  console.warn("[Zuper Confirm] Sibling API search failed:", apiErr);
                }
              }

              if (siblingMap.size > 0) {
                console.log(`[Zuper Confirm] Found ${siblingMap.size} sibling construction job(s) to reschedule`);
