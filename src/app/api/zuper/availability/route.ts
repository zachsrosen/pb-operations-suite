...
interface CrewSchedule {
  ...
  location: string; // "DTC", "Westminster", "Pueblo", etc.
...
export async function GET(request: NextRequest) {
      ...
      for (const job of jobsResult.data) {
        if (job.scheduled_start_time && !isEffectivelyUnscheduled(job)) {
          ...
          if (availabilityByDate[dateStr]) {
            ...
            // Extract address + coordinates for travel-time annotation
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const customerAddr = (job as any).customer_address;
            const jobAddress = customerAddr
              ...
              : undefined;
            const jobGeo = customerAddr?.geo_coordinates?.latitude
  ...
  if (travelConfig.enabled && type === "survey" && candidateAddress) {
    for (const dateStr in availabilityByDate) {
      ...
      // Group booked slots by user_uid (primary) or normalized name (fallback)
      const bookedByUser: Record<string, Array<{
        start_time: string; end_time: string;
        address?: string; geoCoordinates?: { latitude: number; longitude: number };
        projectName?: string;
      }>> = {};
      ...
      for (const k in bookedByUser) {
        bookedByUser[k].sort((a: { start_time: string }, b: { start_time: string }) => a.start_time.localeCompare(b.start_time));
      }

      // Annotate available slots with travel warnings
