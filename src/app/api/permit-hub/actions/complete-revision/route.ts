...
export async function POST(req: NextRequest) {
  ...
  const noteBody = [
    `<b>Revision complete — ready to resubmit</b>`,
  ...
  const result = await completePermitTask({
    dealId: p.dealId,
    actionKind: "COMPLETE_REVISION",
    noteBody,
    fallbackProperties: {
      permitting_status: "Returned from Design",
    },
  ...
  await recordPermitActivity({
    ...
    userName: auth.name,
    type: "PERMIT_REVISION_COMPLETED",
    dealId: p.dealId,
    description: "Marked revision complete",
    metadata: { ...p, taskCompleted: result.taskCompleted },
  });

  if (userId) {
    await deletePermitDraft({ userId, dealId: p.dealId, actionKind: "COMPLETE_REVISION" });
  }
