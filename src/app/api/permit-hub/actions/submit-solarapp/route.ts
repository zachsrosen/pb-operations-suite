...
const Schema = z.object({
  ...
  solarAppProjectNumber: z.string().min(1),
...
export async function POST(req: NextRequest) {
  ...
  const noteBody = [
    `<b>Submitted SolarApp+</b>`,
    `Date: ${p.submissionDate}`,
    `SolarApp+ project #: ${p.solarAppProjectNumber}`,
  ...
  await recordPermitActivity({
    ...
    userName: auth.name,
    type: "PERMIT_SOLARAPP_SUBMITTED",
    dealId: p.dealId,
    description: `Submitted SolarApp+ (project ${p.solarAppProjectNumber})`,
    metadata: { ...p, taskCompleted: result.taskCompleted },
  });

  if (userId) {
    await deletePermitDraft({ userId, dealId: p.dealId, actionKind: "SUBMIT_SOLARAPP" });
  }
