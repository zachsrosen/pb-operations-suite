/**
 * Ticket BOM History — All Snapshots (Service Suite)
 *
 * GET /api/bom/ticket-history/all
 *   Returns Ticket BOM snapshots across all tickets, newest-first,
...
export async function GET(request: NextRequest) {
  ...
  const raw = await prisma.ticketBomSnapshot.findMany({
  ...
  // Map to the same wire shape as /api/bom/history/all for UI compatibility:
  // dealId/dealName slots are filled with ticketId/ticketSubject so the
  // existing BOM history UI can render either kind without branching.
