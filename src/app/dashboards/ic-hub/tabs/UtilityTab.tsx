...
export function UtilityTab({ utility }: { utility: UtilityRecord[] }) {
  if (!utility.length) {
    return (
        ...
        return (
          <div key={record.id} className="rounded-lg border border-t-border p-4">
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
              ...
              <Field
                label="Typical IC turnaround"
                value={p.interconnection_turnaround_time}
              />
              ...
              <Field label="PTO turnaround" value={p.pto_turnaround_time} />
            </dl>
            {p.interconnection_issues && (
              <div className="mt-4 rounded-md bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-300">
                <strong>Known issues:</strong> {p.interconnection_issues}
              </div>
            )}
            {p.general_notes && (
