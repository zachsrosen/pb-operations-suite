...
interface Props {
  ...
  threads: IcProjectDetail["correspondenceThreads"];
  inbox: string | null;
}
    ...
    <div className="space-y-4">
      <div className="text-muted flex items-center justify-between text-xs">
        <span>
          {inbox ? (
            <>
              Showing last {threads.length} thread
              {threads.length === 1 ? "" : "s"} from{" "}
              <span className="font-medium">{inbox}</span>
            </>
          ) : (
            "Shared IC inbox not configured for this region — showing Gmail search link only."
          )}
        </span>
        {searchUrl && (
          <a
