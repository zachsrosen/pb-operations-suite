...
export function FormShell<TPayload extends Record<string, unknown>>({
  ...
  const draftQuery = useQuery<{
    ...
    queryKey: queryKeys.icHub.draft(dealId, actionKind),
    queryFn: async () => {
      const r = await fetch(`/api/ic-hub/drafts/${dealId}/${actionKind}`);
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: queryKeys.icHub.queue() });
      qc.invalidateQueries({ queryKey: queryKeys.icHub.project(dealId) });
      qc.invalidateQueries({ queryKey: queryKeys.icHub.todayCount() });
