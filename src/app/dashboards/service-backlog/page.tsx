...
interface ServiceEquipmentResponse {
  projects: Project[];
...
const SERVICE_STAGE_COLORS: Record<string, { tw: string; hex: string }> = {
  "Project Preparation": { tw: "bg-blue-500", hex: "#3B82F6" },
  "Site Visit Scheduling": { tw: "bg-purple-500", hex: "#A855F7" },
  "Work In Progress": { tw: "bg-orange-500", hex: "#F97316" },
  "Inspection": { tw: "bg-amber-500", hex: "#F59E0B" },
  "Invoicing": { tw: "bg-pink-500", hex: "#EC4899" },
  "Completed": { tw: "bg-green-500", hex: "#22C55E" },
  "Cancelled": { tw: "bg-red-500", hex: "#EF4444" },
};
...
function buildProductSummary(
  ...
  // Sort by stage urgency (closest to Work In Progress first), then by count
  const stageUrgency = (proj: ProductSummary) => {
    let minIdx = 999;
    for (const p of proj.projectList) {
      const idx = SERVICE_STAGE_ORDER.indexOf(p.stage as (typeof SERVICE_STAGE_ORDER)[number]);
  ...
  return [...map.values()].sort((a, b) => {
    ...
    if (urgA !== urgB) return urgB - urgA; // Closer to completion first (higher index)
...
export default function ServiceBacklogPage() {
  useActivityTracking();
  ...
  const { data, isLoading, isError, refetch } = useQuery<ServiceEquipmentResponse>({
    queryKey: queryKeys.projects.list({ context: "service-equipment" }),
    queryFn: async () => {
      const res = await fetch("/api/service/equipment");
  ...
  const stages = useMemo(
    () =>
      [...new Set(projects.map((p) => p.stage))]
        .filter(Boolean)
        .sort((a, b) => {
          const ai = SERVICE_STAGE_ORDER.indexOf(a as (typeof SERVICE_STAGE_ORDER)[number]);
          const bi = SERVICE_STAGE_ORDER.indexOf(b as (typeof SERVICE_STAGE_ORDER)[number]);
    ...
    return [...displayProjects].sort((a, b) => {
      ...
      switch (sortField) {
        case "modules":
          ...
          break;
        case "stage":
          cmp = (SERVICE_STAGE_ORDER.indexOf(a.stage as (typeof SERVICE_STAGE_ORDER)[number]) ?? 999) -
                (SERVICE_STAGE_ORDER.indexOf(b.stage as (typeof SERVICE_STAGE_ORDER)[number]) ?? 999);
          break;
        case "location":
  ...
  const stageBreakdown = useMemo(() => {
    ...
    return [...map.entries()]
      .sort((a, b) => {
        const ai = SERVICE_STAGE_ORDER.indexOf(a[0] as (typeof SERVICE_STAGE_ORDER)[number]);
        const bi = SERVICE_STAGE_ORDER.indexOf(b[0] as (typeof SERVICE_STAGE_ORDER)[number]);
        return (ai === -1 ? 999 : ai) - (bi === -1 ? 999 : bi);
      });
  }, [filteredProjects]);
  ...
  if (isLoading) {
    return (
      <DashboardShell title="Service Equipment Backlog" accentColor="purple">
    ...
    return (
      <DashboardShell title="Service Equipment Backlog" accentColor="purple">
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          <p className="text-red-400">Failed to load service equipment data. Please try refreshing.</p>
          <button
            onClick={() => refetch()}
  ...
  const exportFilename = activeStatFilter
    ? `service-equipment-${activeStatFilter === "in_progress" ? "in-progress" : activeStatFilter}`
  ...
  return (
    <DashboardShell
      ...
      subtitle={`${backlogTotals.projects} backlog \u2022 ${inProgressTotals.projects} in progress`}
      accentColor="purple"
        ...
        <div className="ml-auto flex bg-surface-2 rounded-lg p-0.5">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
              view === "summary" ? "bg-purple-600 text-white" : "text-muted hover:text-foreground"
            }`}
          >
            Summary
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
              view === "projects" ? "bg-purple-600 text-white" : "text-muted hover:text-foreground"
            }`}
          >
            Projects
          </button>
                          ...
                          <td className="py-2 pr-4">
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                ...
                                style={{ backgroundColor: SERVICE_STAGE_COLORS[stage]?.hex || "#71717A" }}
                              />
                              <span>{stage}</span>
                              {cls === "in_progress" && <span className="text-[10px] text-orange-400 font-medium ml-1">IN PROGRESS</span>}
                              {d.missing > 0 && (
                                <span className="text-[10px] text-amber-400/80 ml-1" title={`${d.missing} jobs missing equipment data`}>
                                  ⚠ {d.missing}
                                </span>
                              )}
                            </div>
                            ...
                            <td colSpan={8} className="px-0 py-0">
                              <div className="bg-surface-2/20 border-l-2 border-t-border/50 ml-6 px-4 py-3 mb-1">
                                ...
                                <div className="border-t border-t-border/30 pt-2">
                                  <div className="text-[10px] text-muted/60 uppercase tracking-wide mb-1">Jobs</div>
                        ...
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-1">
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="block font-medium text-foreground/90 hover:text-purple-400 truncate max-w-[220px] transition-colors"
                            >
                              {p.name}
                            </a>
                            {isMissing && (
                        ...
                        <td className="px-4 py-2.5">
                          <span className="inline-flex items-center gap-1.5 text-xs">
                            <span
                              ...
                              style={{ backgroundColor: SERVICE_STAGE_COLORS[p.stage]?.hex || "#71717A" }}
                            />
