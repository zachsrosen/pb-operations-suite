...
export default function ZuperStatusComparisonPage() {
  ...
  const SortIcon = ({ field }: { field: string }) => {
    if (sortField !== field) return <span className="text-muted dark:text-muted/70 ml-1">&uarr;&darr;</span>;
    ...
    return (
      ...
      >
        <div className="flex items-center justify-center py-20">
          ...
          <span className="ml-3 text-muted dark:text-muted">
  ...
  return (
    ...
    >
            ...
            <button
              ...
              className={`rounded-xl border p-4 text-left transition-all ${
                activeCategory === cat
                  ...
                  : "border-t-border dark:border-t-border hover:border-t-border dark:hover:border-t-border"
              } ${CATEGORY_BG[cat]}`}
            >
              <div className="flex items-center justify-between mb-2">
                ...
                {activeCategory === cat && (
                  <span className="text-xs text-muted dark:text-muted">Active</span>
                )}
              </div>
              <div className="flex items-end justify-between mb-2">
                <div>
                  <div className="text-xl font-bold">{catStats?.total || 0} jobs</div>
                  <div className="text-xs text-muted dark:text-muted space-x-2">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-muted dark:text-muted">status match</div>
                </div>
              </div>
              <div className="h-1.5 bg-zinc-200 dark:bg-surface-2 rounded-full overflow-hidden">
                <div
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        {/* View mode toggles */}
        <div className="flex rounded-lg border border-t-border dark:border-t-border overflow-hidden">
          {([
            { key: "all", label: "All" },
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium transition-colors ${
                viewMode === key
                  ...
                  : "bg-white dark:bg-surface text-muted/70 dark:text-muted hover:bg-zinc-50 dark:hover:bg-surface-2"
              }`}
            >
              {label}
            </button>
        ...
        <div className="relative flex-1 min-w-[200px] max-w-md">
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-lg border border-t-border dark:border-t-border bg-white dark:bg-surface text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500"
          />
              ...
              <button
                ...
                className={`px-2 py-1 rounded-md text-xs border transition-colors ${
                  isSelected
                    ...
                    : "bg-zinc-50 dark:bg-surface border-t-border dark:border-t-border text-muted/70 dark:text-muted hover:border-t-border dark:hover:border-muted"
                }`}
        ...
        <label className="flex items-center gap-2 text-xs cursor-pointer select-none">
          <input
            ...
            className="rounded border-t-border dark:border-t-border text-red-600 focus:ring-red-500"
          />
          <span className="text-muted/50 dark:text-foreground/80">Status mismatches</span>
        </label>
        ...
        <label className="flex items-center gap-2 text-xs cursor-pointer select-none">
          <input
            ...
            className="rounded border-t-border dark:border-t-border text-orange-600 focus:ring-orange-500"
          />
          <span className="text-muted/50 dark:text-foreground/80">Date mismatches</span>
        </label>
        ...
        <span className="ml-auto text-xs text-muted dark:text-muted">
      ...
      {/* Project View Tables */}
      {isProjectView && (
        <div className="rounded-xl border border-t-border dark:border-t-border overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-zinc-50 dark:bg-skeleton border-b border-t-border dark:border-t-border">
                  <th className="px-3 py-2.5 text-left font-medium text-muted/70 dark:text-foreground/80 text-xs sticky left-0 bg-zinc-50 dark:bg-skeleton z-10">
                    Project
                  </th>
                  {viewMode === "project-status" ? (
                    <>
                      <th colSpan={3} className="px-2 py-1 text-center font-medium text-xs border-l border-t-border dark:border-t-border">
                    ...
                    <>
                      <th colSpan={5} className="px-2 py-1 text-center font-medium text-xs border-l border-t-border dark:border-t-border">
                        <span className={`px-2 py-0.5 rounded-full ${CATEGORY_BADGE.site_survey}`}>Site Survey</span>
                      </th>
                      <th colSpan={5} className="px-2 py-1 text-center font-medium text-xs border-l border-t-border dark:border-t-border">
                        <span className={`px-2 py-0.5 rounded-full ${CATEGORY_BADGE.construction}`}>Construction</span>
                  ...
                  <th className="px-3 py-2.5 text-center font-medium text-muted/70 dark:text-foreground/80 text-xs border-l border-t-border dark:border-t-border">
                    Links
                  </th>
                </tr>
                <tr className="bg-zinc-50/50 dark:bg-surface-2/30 border-b border-t-border dark:border-t-border text-[10px] font-medium text-muted dark:text-muted uppercase tracking-wider">
                  <th className="px-3 py-1.5 text-left sticky left-0 bg-zinc-50/50 dark:bg-surface-2/30 z-10">&nbsp;</th>
                  {viewMode === "project-status" ? (
                    <>
                      {/* Survey status sub-headers */}
                      <th className="px-2 py-1.5 text-left border-l border-t-border dark:border-t-border">Zuper</th>
                    ...
                    <>
                      ...
                      <th className="px-2 py-1.5 text-left border-l border-t-border dark:border-t-border">Z Sched</th>
                  ...
                  <th className="px-3 py-1.5 border-l border-t-border dark:border-t-border">&nbsp;</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100 dark:divide-t-border">
                {filteredProjectRecords.length === 0 ? (
                  <tr>
                    <td colSpan={20} className="px-4 py-12 text-center text-muted dark:text-muted">
                  ...
                  filteredProjectRecords.map((rec) => (
                    ...
                    >
                      <td className="px-3 py-2 sticky left-0 bg-white dark:bg-surface z-10">
                        <div className="font-mono font-medium text-foreground dark:text-foreground text-xs">
                          {rec.projectNumber}
                        </div>
                        {rec.dealName && (
                          <div className="text-[10px] text-muted dark:text-muted truncate max-w-[140px]" title={rec.dealName}>
                      ...
                      <td className="px-3 py-2 text-center border-l border-t-border dark:border-t-border">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-zinc-50 dark:bg-skeleton border-b border-t-border dark:border-t-border">
                ...
                <th className="px-3 py-2.5 text-left font-medium text-muted/70 dark:text-foreground/80 cursor-pointer hover:text-foreground dark:hover:text-foreground text-xs" onClick={() => handleSort("category")}>
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-t-border">
              {filteredRecords.length === 0 ? (
                <tr>
                  <td colSpan={15} className="px-4 py-12 text-center text-muted dark:text-muted">
                    No records found matching your filters.
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        ...
                        {record.dealName && (
                          <div className="text-[10px] text-muted dark:text-muted truncate max-w-[160px]" title={record.dealName}>
                      ...
                      <td className="px-3 py-2.5">
                        {record.team && (
                          <div className="text-[10px] text-muted/70 dark:text-muted truncate max-w-[100px]" title={record.team}>
                            {record.team}
                          </div>
                        )}
                        {record.assignedTo && (
                          <div className="text-[10px] text-muted truncate max-w-[100px]" title={record.assignedTo}>
                            {record.assignedTo}
                          </div>
                        )}
                        {!record.team && !record.assignedTo && <span className="text-[10px] text-muted">-</span>}
                      </td>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {(["site_survey", "construction", "inspection"] as CategoryKey[]).map((cat) => {
              ...
              return (
                <div key={cat} className={`rounded-xl border border-t-border dark:border-t-border p-4 ${CATEGORY_BG[cat]}`}>
                  ...
                  {statusMismatches.length > 0 && (
                    <div className="mb-3">
                      <div className="text-[10px] font-semibold text-muted dark:text-muted uppercase tracking-wider mb-1.5">
                      ...
                      <div className="space-y-1">
                        {sortedPairs.slice(0, 5).map(([pair, count]) => (
                          <div key={pair} className="flex items-center justify-between text-xs">
                            <span className="text-muted/70 dark:text-muted truncate mr-2">{pair}</span>
                            <span className="font-mono font-medium text-foreground dark:text-foreground flex-shrink-0">{count}</span>
                          </div>
                        ))}
                      </div>
                    </div>
...
function StatCard({ label, value, color }: { label: string; value: number; color?: string }) {
  ...
  return (
    <div className={`rounded-xl border p-3 ${color ? colorClasses[color] : "border-t-border dark:border-t-border bg-white dark:bg-surface"}`}>
      <div className={`text-xs ${color ? textClasses[color] : "text-muted dark:text-muted"}`}>{label}</div>
...
function StatusBadge({ status }: { status: string }) {
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-surface-2 text-foreground/90 dark:text-foreground/90 text-[11px] font-medium">
...
function ProjectStatusCells({ slot }: { slot: CategorySlot }) {
  if (!slot.zuperStatus) {
    return (
      <>
        <td className="px-2 py-2 border-l border-t-border dark:border-t-border">
          <span className="text-[10px] text-muted italic">No job</span>
        </td>
        <td className="px-2 py-2"><span className="text-[10px] text-muted">-</span></td>
        <td className="px-2 py-2 text-center"><span className="text-[10px] text-muted">-</span></td>
      </>
    );
  }
  return (
    <>
      ...
      <td className="px-2 py-2">
        {slot.hubspotStatus ? (
          ...
          <span className="text-[10px] text-muted italic">Not set</span>
        )}
      </td>
