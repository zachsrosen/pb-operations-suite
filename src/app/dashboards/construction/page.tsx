...
export default function ConstructionDashboardPage() {
  ...
  const [filterConstructionStatuses, setFilterConstructionStatuses] = useState<string[]>([]);
  ...
  }, [projects, filterLocations, filterStages, filterConstructionStatuses, searchQuery, isInConstructionPhase]);
  ...
  const stages = useMemo(() => {
    ...
    return [...new Set(projects.filter(isInConstructionPhase).map(p => p.stage))]
      .filter(s => s)
      .sort((a, b) => {
      ...
      <DashboardShell title="Construction" accentColor="orange">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="text-zinc-400">Loading Construction Data...</p>
          </div>
        </div>
      </DashboardShell>
    );
    ...
    <DashboardShell title="Construction" accentColor="orange">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-[#12121a] rounded-xl p-4 border border-zinc-800">
          ...
          <div className="text-sm text-zinc-400">Ready To Build</div>
          <div className="text-xs text-zinc-500">{formatMoney(stats.readyToBuild.reduce((s, p) => s + (p.amount || 0), 0))}</div>
        </div>
        <div className="bg-[#12121a] rounded-xl p-4 border border-zinc-800">
          <div className="text-2xl font-bold text-yellow-400">{stats.inConstruction.length}</div>
          <div className="text-sm text-zinc-400">In Construction</div>
          <div className="text-xs text-zinc-500">{formatMoney(stats.inConstruction.reduce((s, p) => s + (p.amount || 0), 0))}</div>
        </div>
        <div className="bg-[#12121a] rounded-xl p-4 border border-zinc-800">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-[#12121a] rounded-xl p-4 border border-zinc-800">
          ...
          <div className="text-sm text-zinc-400">Avg Days in Construction</div>
        </div>
        <div className="bg-[#12121a] rounded-xl p-4 border border-zinc-800">
        ...
        <div className="bg-[#12121a] rounded-xl p-4 border border-zinc-800">
          ...
          <div className="text-sm text-zinc-400">Blocked/Rejected</div>
        </div>
      </div>
      ...
      <div className="bg-[#12121a] rounded-xl border border-zinc-800 p-4 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
          {Object.keys(stats.constructionStatusStats).length === 0 ? (
            <p className="text-zinc-500 text-sm col-span-full">No construction status data available</p>
          ) : (
      ...
      <div className="bg-[#12121a] rounded-xl border border-zinc-800 overflow-hidden">
        <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
          ...
          {hasActiveFilters && (
            <span className="text-xs text-zinc-500">Filtered from {projects.filter(isInConstructionPhase).length} total</span>
          )}
        </div>
          ...
          <table className="w-full">
            <thead className="bg-zinc-900">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-zinc-400 uppercase">Construction Status</th>
                ...
                filteredProjects
                  ...
                  .map(project => (
                    <tr key={project.id} className="hover:bg-zinc-900/50">
                      ...
                      <td className={`px-4 py-3 text-sm ${project.constructionScheduleDate ? 'text-blue-400' : 'text-zinc-500'}`}>
                        {project.constructionScheduleDate || '-'}
                      </td>
                      <td className={`px-4 py-3 text-sm ${project.constructionCompleteDate ? 'text-green-400' : 'text-zinc-500'}`}>
