  ...
  const content = (
    <>
      {value === null ? (
        ...
        <div key={String(value)} className={`font-bold text-foreground mb-1 tracking-tight tabular-nums animate-value-flash ${String(value).length > 10 ? "text-xl xl:text-2xl" : String(value).length > 7 ? "text-2xl xl:text-3xl" : "text-3xl xl:text-4xl"}`}>
          {value}
        </div>
      )}
