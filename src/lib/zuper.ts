...
// Pure category constants live in a Prisma-free module so client components can
// import them without pulling the DB client into the browser bundle. Imported
// here for internal use and re-exported below for back-compat.
import {
  JOB_CATEGORY_UIDS,
  JOB_CATEGORIES,
  CONSTRUCTION_JOB_SPLIT_ENABLED,
  CONSTRUCTION_CATEGORY_UIDS,
  CONSTRUCTION_CATEGORY_NAMES,
} from "@/lib/zuper-categories";
...
// Job-category constants moved to the Prisma-free `@/lib/zuper-categories`
// module (so client components can import them without bundling the DB client).
// Re-exported here so existing `@/lib/zuper` importers keep working unchanged.
export {
  JOB_CATEGORY_UIDS,
  JOB_CATEGORIES,
  CONSTRUCTION_JOB_SPLIT_ENABLED,
