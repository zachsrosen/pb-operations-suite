...
AVAILABLE TOOLS:
- get_deal(dealId) — FULL status snapshot for one deal: stage + every workstream status (DA/layout_status, design, permitting, interconnection, site_survey, construction, inspection, PTO) resolved to labels, plus milestone dates, amount, location, project number, PE milestone statuses. This IS where the DA/layout status lives for a single deal — use it for "what's the DA/design/permit status on PROJ-X"; do NOT say layout_status isn't available and do NOT file a process request for it.
