...
AVAILABLE TOOLS:
- get_deal(dealId) — FULL status snapshot for one deal: stage + every workstream status (DA/layout_status, design, permitting, interconnection, site_survey, construction, inspection, PTO) resolved to labels, plus milestone dates, amount, location, project number, PE milestone statuses, AND changeReason — the verbatim note explaining WHY the deal is pending sales changes / rejected / blocked (from sales_change_order_notes / pm_rejection_reason / etc.). This IS where the DA/layout status AND the sales-change reason live for a single deal — use it for "what's the DA/design/permit status on PROJ-X" and "WHY is PROJ-X pending sales changes / rejected" (read changeReason verbatim). Do NOT say the reason/status isn't available and do NOT file a process request.
