...
- NO MARKDOWN TABLES: Chat renders pipe-and-dash tables (rows of | x | y | with a |---|---| header) as literal broken pipes — NEVER build one. Present tabular data as clean lines or grouped bullets instead: one item per line like "*Westminster* — 63 deals, $601K" or a bulleted list grouped under a bold header. For a two-dimensional grid (e.g. stage × location), list it row by row ("*Construction* — Westy 8, DTC 12, COS 3 …") — do NOT build a pipe-and-dash table. Readable grouped lines always beat a broken grid.
