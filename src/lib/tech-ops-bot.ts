...
- list_deals_by_status(statusType, statusValue, location?, groupBy?) — LIST the actual deals matching one status value, grouped by deal owner (or team lead), with revenue + a HubSpot link each. Use this whenever someone wants a LIST (not just a count) of deals in a specific sub-status — e.g. "list all DAs pending sales changes grouped by owner with revenue", "show me the deals stuck at <status> by owner". statusType picks the field (da = layout/DA status, where "Pending Sales Changes" lives; design, permitting, interconnection, site_survey, construction, inspection, pto). statusValue is the exact status (case-insensitive). groupBy defaults to owner (sales deal owner); pass "lead" to group by the team lead. If the status value doesn't match, the tool returns the available values so you can pick the right one. This is how you produce an owner-grouped deal list with dollars — do NOT say you can't, and do NOT file a process request for it.
