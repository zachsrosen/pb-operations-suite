...
- query_projects(filters?, groupBy?, includeInactive?) — THE GENERAL deal query. Use it for ANY "list/count deals where X, optionally grouped by Y, with revenue" question — any combination of status, stage, location, owner, lead, PE, or amount, grouped however they ask, in whatever format. filters is an array of {field, op, value}, all ANDed. Fields: stage, da_status (layout/DA — where "Pending Sales Changes" lives), design_status, permitting_status, interconnection_status, site_survey_status, construction_status, inspection_status, pto_status, location, owner (sales deal owner), design_lead, permit_lead, interconnection_lead, inspection_lead, project_manager, surveyor, participate_energy (true/false), amount, project_number. Ops: equals, in (value=array), not, contains, gt, lt, gte, lte, present, blank (status/name matching is case-insensitive, accepts label or raw value). groupBy = any of those fields → returns a per-group count+revenue rollup. Returns total, totalRevenue, byGroup, and the deal list (with owner/leads/stage/revenue + HubSpot url). Examples: "list all DAs pending sales change by owner with revenue" → filters:[{field:"da_status",op:"equals",value:"Pending Sales Changes"}], groupBy:"owner". "how many PE deals in construction by location" → filters:[{field:"stage",op:"equals",value:"Construction"},{field:"participate_energy",op:"equals",value:true}], groupBy:"location". If a value matches nothing the tool returns the available values — pick the closest, NEVER fabricate. This is how you answer these directly — do NOT say you can't and do NOT file a process request.
