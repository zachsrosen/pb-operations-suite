...
export const UPDATES: UpdateEntry[] = [
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "improvement", text: "Catalog product listing replaced with a responsive card layout for better usability on narrow screens" },
  ...
  },
  {
    ...
    description: "Construction planning gained week/gantt views plus a tentative optimizer workflow, BOM customer matching now uses HubSpot primary-contact intelligence to auto-select Zoho customers safely, and catalog/admin workflows were hardened with richer product management and post-processing safeguards.",
    changes: [
      ...
      { type: "feature", text: "Catalog admin capabilities expanded with full internal product edit surface and metadata-aware update handling for category-specific fields (#42)" },
      ...
      { type: "internal", text: "Published new design docs for catalog product hard-delete implementation and QuickBooks product seeding strategy; forced redeploy to clear stale build cache" },
    ],
  },
  {
    version: "1.57.0",
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "fix", text: "Inventory/BOM safety hardening added legacy schema fallback for product lookup and protected against enum-drift mismatches in production" },
  ...
  },
  {
    ...
    changes: [
      { type: "feature", text: "New Equipment Catalog dashboard (`/dashboards/catalog`) with product visibility, sync status indicators, stock rollups, and a pending approval review queue" },
  ...
  },
  {
    ...
    changes: [
      { type: "feature", text: "Extended schema for system sync: added `zohoItemId` on `InternalProduct` and `zohoPoId` on `ProjectBomSnapshot`, plus migration support for both columns" },
      ...
      { type: "feature", text: "New authenticated `POST /api/bom/create-po` endpoint to generate Zoho draft POs from `(dealId, version, vendorId)`, then persist resulting `zohoPoId` back to the BOM snapshot" },
      ...
      { type: "fix", text: "Create-PO line item builder now filters non-inventory BOM categories before Prisma enum casting, preventing runtime validation failures for categories not stored in `InternalProduct`" },
      ...
      { type: "fix", text: "BOM history panel load now restores stored `zohoPoId` correctly so PO state survives reloads and snapshot switching" },
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "feature", text: "Product auto-sync: one-click catalog population from HubSpot projects — extracts module, inverter, battery, and EV charger data" },
      ...
      { type: "feature", text: "Four summary stat cards: Total Products, Total Units, Items Below Demand, and Locations Tracked" },
      ...
      { type: "internal", text: "New Prisma models: InternalProduct, InventoryStock, StockTransaction with full indexing" },
