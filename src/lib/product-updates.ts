...
export const UPDATES: UpdateEntry[] = [
  {
    version: "1.64.0",
    date: "2026-03-23",
    title: "Metrics Dashboard Suite, Cross-System Sync Relay, and Role Rename",
    description: "Five new operational metrics dashboards land across survey, construction, inspection, design approval, and preconstruction — each with turnaround stats, drill-downs, and action queues. The catalog SyncModal is rewritten with a plan-based relay engine supporting per-cell source selection across Zoho, HubSpot, and Zuper. OWNER role renames to EXECUTIVE with SALES_MANAGER added, suite pages get a visual polish pass with emoji icons and accent bars, and DashboardShell gains suite-aware breadcrumb navigation.",
    changes: [
      { type: "feature", text: "Site Survey Turnaround Metrics dashboard — avg turnaround by office and surveyor, drill-down to individual deals with Zuper/HubSpot links, split awaiting table into Past Due (red) and Upcoming sections, sortable columns, and location filtering" },
      { type: "feature", text: "Construction Completion Metrics dashboard — avg RTB-to-Schedule, construction duration, RTB-to-CC, and CC-to-Inspection Passed times by office with drill-down tables, Zuper deep links, and in-construction project table" },
      { type: "feature", text: "Inspection Metrics dashboard with dual-source validation — per-location and per-AHJ turnaround breakdowns, computed vs HubSpot rollup validation, Outstanding Failed Inspections and CC Pending Inspection action queues, Location custom object and 11 new inspection deal properties" },
      { type: "feature", text: "Design Approval Metrics dashboard — DA turnaround and revision stats by office, first-try approval rate, Survey Complete to DA Not Sent pipeline, DA Sent to Pending Approval pipeline, location filter, and sortable columns" },
      { type: "feature", text: "Preconstruction Metrics dashboard — split Permitting and Interconnection sections, filter persistence via zustand store, monthly bar charts, error state with retry, and Executive Suite card" },
      { type: "feature", text: "Cross-system Sync Relay engine — plan-based flow replacing push/pull/skip cycling, snapshot builder with default intents, conflict detection with deterministic winners, HMAC-signed plan hash confirmation tokens, stale detection on execute, and per-field source attribution in outcomes" },
      { type: "feature", text: "SyncModal rewrite — wide 4-column comparison table with per-cell source selection dropdowns, product search and linking for unlinked systems, always-visible Create button, companion field auto-inclusion (vendor_id with vendor_name), and relay-only pull hint" },
      { type: "feature", text: "Availability change request approval workflow — AvailabilityChangeRequest Prisma model, crew changes routed through manager approval instead of direct mutation, admin approval dashboard with Pending/Approved/Rejected tabs, email notification template, and transactional approval execution" },
      { type: "feature", text: "Execution/Metrics table reshuffle — shared useSort hook and SortHeader/DealLinks/format-helpers extracted, failed/pending inspection tables moved to inspections execution page, past-due/upcoming survey tables moved to site-survey execution page, scope=pipeline API fast path for inspection-metrics" },
      { type: "improvement", text: "Suite page visual polish (Phase 1) — emoji icons on all suite cards, SUITE_ACCENT_COLORS and SECTION_COLORS maps with accent-bar section headers, Open arrow affordance, disabled card support, and deprecated tagColor/hoverBorderClass props removed across all 8 suites" },
      { type: "improvement", text: "DashboardShell chrome polish — PhotonBrothersBadge linking to parent suite, suite accent left border on title block, suite-colored breadcrumb links, indigo/teal colorMap entries, isRealSuite guard, and mobile flex-wrap stacking" },
      { type: "improvement", text: "MetricCard tier differentiation — StatCard (hero) with text-4xl and deeper gradients, MetricCard (standard) with href and null value support, MiniStat (compact) with reduced padding, indigo + teal accent colors added, SummaryCard removed" },
      { type: "improvement", text: "DA metrics consolidated into D&E Metrics page with time-window toggle, Pending Approval restructured into Design Approval Queue with 3 pre-filtered action sections, Operations suite reorganized from function-based to workflow-based sections" },
      { type: "improvement", text: "Zuper status comparison accuracy — completion date enrichment from job status history (removed 50-job cap), superseded inspection detection, date drift indicator, duplicate active job detection, 6-month audit window, multi-select Zuper/HubSpot/deal-stage filters, and row health borders" },
      { type: "improvement", text: "Construction scheduler revenue cards — per-status revenue breakdown, completed month/year stats, overdue revenue display, and formatCurrency guard against undefined values" },
      { type: "improvement", text: "10 new catalog sync mapping edges — zoho part_number and unit made bidirectional, selectionToIntents translation layer with smart defaults and dropdown filtering, updated Zoho/HubSpot/Zuper field parsers" },
      { type: "fix", text: "OWNER enum deserialization error (460 Sentry events) — OWNER restored as legacy Prisma enum value so generated client accepts both OWNER and EXECUTIVE from database before normalizeRole() runs" },
      { type: "fix", text: "Edge-runtime JWT role stuck at VIEWER — middleware now falls back to pb_effective_role cookie when JWT is VIEWER, AuthSync triggers session.update() to force JWT refresh from Node.js context" },
      { type: "fix", text: "Catalog sync confirmation token — execute endpoint validates HMAC against client-provided changesHash instead of recomputing from non-deterministic external API responses; stale detection returns 409 when external data changed between preview and execute" },
      { type: "fix", text: "External product link URLs — Zoho URLs updated to org-ID path with hash prefix, Zuper product URLs corrected to /products/{uid}/details, HubSpot product URLs updated to product library list, NEXT_PUBLIC_ env vars with explicit CLIENT_ENV map for client-side inlining" },
      { type: "fix", text: "HubSpot manufacturer enum case mismatch — diffFields accepts caseInsensitiveFields option so HYUNDAI vs Hyundai no longer surfaces as a change; Zuper update payload format corrected for product API field names" },
      { type: "fix", text: "Zuper product categories — 6 missing categories created via API (Module, EV Charger, Optimizer, D&R, Service, Tesla System Components), fallback map updated to all 14 UIDs, MODULE zuperCategory renamed from Solar Panel to Module" },
      { type: "fix", text: "Inspection turnaround stored in milliseconds — parseMsToDays conversion applied to inspection and construction turnaround fields in Location/AHJ custom object rollup extractors" },
      { type: "fix", text: "Survey metrics date classification — schedule dates parsed as local time to prevent same-day surveys showing as past due; Zuper job fetch expanded to include awaiting-survey deal IDs" },
      { type: "internal", text: "OWNER role renamed to EXECUTIVE with Prisma migration, SALES_MANAGER role added with permissions, normalizeRole() extended for OWNER to EXECUTIVE mapping, EXECUTIVE added to sop-access ADMIN_ROLES and design/install review allowed roles" },
      { type: "internal", text: "Generator abstraction removed from catalog sync — SyncModal simplified by 400+ lines, mapping table and plan engine cleaned of generator logic, tests updated" },
      { type: "internal", text: "Core dependency upgrade (66 packages updated), shared useSort/SortHeader/DealLinks/format-helpers extracted, locations.ts module added for access-scope, and 100+ new test cases across sync relay, sort hooks, and metric cards" },
    ],
  },
  {
    version: "1.63.0",
