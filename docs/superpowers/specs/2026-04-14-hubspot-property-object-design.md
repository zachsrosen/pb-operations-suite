...
| 2 | **Canonical identity = Google `place_id`, with normalized address + lat/lng as secondary** | `place_id` is stable across re-geocodes and handles unit-level granularity. Normalized address is fallback for rural addresses where Google returns no `place_id`. |
...
- `src/app/api/webhooks/property/route.ts` — webhook receiver with HubSpot signature verification
...
- `src/app/api/cron/property-reconcile/route.ts` — nightly drift repair
...
**Object name**: `Property` (internal: `p21710069_Property`, following AHJ/Utility pattern)
...
model HubSpotPropertyCache {
  ...
  // Dedup keys
  googlePlaceId      String?  @unique           // canonical; null only for rural-geocode fallbacks
  normalizedAddress  String                     // fallback dedupe + search
...
Rollup computation reads from existing sources:
- Install dates, counts, system size, battery/EV flags → query Deals + HubSpot line items
